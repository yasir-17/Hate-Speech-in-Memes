...
class InpaintDataset(Dataset):
    def __init__(self):
        self.imglist = []
        for filename in os.listdir(INIMAGE):
            f = os.path.join(INIMAGE, filename)
            if(f[len(f)-1]=='g' and f[len(f)-2]=='n' and f[len(f)-3]=='p' and f[len(f)-4]=='.'):
                self.imglist.append(f)
        self.imglist.sort()
        self.masklist = []
        for filename in os.listdir(MASKIMAGE):
            f = os.path.join(MASKIMAGE, filename)
            if(f[len(f)-1]=='g' and f[len(f)-2]=='n' and f[len(f)-3]=='p' and f[len(f)-4]=='.'):
                self.masklist.append(f)
        self.masklist.sort()
